...
import { isExternalPlugin, loadExternalPlugin } from '../lib/plugin-loader';
import { AssistantPlugin, ClaudePlugin, getCatalogEntry, OllamaPlugin } from '../plugins';
import { assistantMemoryRepository, assistantRepository, conversationRepository } from '../repositories';
...
const ASSISTANT_PLUGIN_ID = 'assistant-builtin';
...
async function loadPlugin(type: string, config: Record<string, unknown>): Promise<PluginBase> {
  // Try external plugins first (from plugins/ directory)
  if (isExternalPlugin(type)) {
    return loadExternalPlugin(type, config);
  }

  // Built-in plugins (only core: assistant, claude, ollama)
  switch (type) {
    case 'assistant':
      return new AssistantPlugin(config);
    case 'claude':
      return new ClaudePlugin(config);
    case 'ollama':
      return new OllamaPlugin(config);
    default:
...
function getToolsForActionIds(actionIds: string[]): { tools: Tool[]; nameToInstanceId: Map<string, string> } {
  ...
  // Always include assistant plugin tools if it's loaded
  const assistantEntry = plugins.get(ASSISTANT_PLUGIN_ID);
  if (assistantEntry?.enabled && hasTools(assistantEntry.plugin)) {
    const pluginTools = assistantEntry.plugin.getTools();
    tools.push(...toTools(pluginTools));
    for (const tool of pluginTools) {
      nameToInstanceId.set(tool.name, ASSISTANT_PLUGIN_ID);
    }
  }
...
/** Wrapper that calls the connector handler service with plugin-manager dependencies */
async function handleConnectorMessage(entry: InstanceEntry, message: Message): Promise<void> {
  return handleConnectorMessageImpl(entry, message, plugins, getToolsForActionIds);
}

/** Wrapper for legacy code that still calls generateWithTools directly */
async function generateWithTools(providerId: string, model: string, messages: LLMMessage[], tools: Tool[], nameToInstanceId: Map<string, string>, actionContext: ToolContext, options?: { temperature?: number; maxTokens?: number }) {
  return generateWithToolsImpl(providerId, model, messages, tools, nameToInstanceId, actionContext, plugins, options);
}
...
export const serverPluginManager = {
  async addPlugin(type: string, config: Record<string, unknown>, instanceId?: string): Promise<PluginInstance> {
    ...
    if (hasConnector(plugin)) {
      ...
      connector.onMessage((message: Message) => {
        handleConnectorMessage(entry, message).catch((err) => logger.error({ connectorId: id, error: (err as Error)?.message }, 'Connector reply failed'));
      });
  ...
  async configurePlugin(instanceId: string, config: Record<string, unknown>): Promise<void> {
    ...
    if (hasConnector(entry.plugin)) {
      ...
      connector.onMessage((message: Message) => {
        handleConnectorMessage(entry, message).catch((err) => logger.error({ connectorId: instanceId, error: (err as Error)?.message }, 'Connector reply failed'));
      });
  ...
  getModelsForProvider(providerId: string): Model[] | null {
    ...
    if (!entry || !hasLLM(entry.plugin)) return null;
  ...
  async generateWithProvider(
    ...
    if (tools?.length && toolNameToInstanceId != null && actionContext != null) {
      const result = await generateWithTools(instanceId, model, messages, tools, toolNameToInstanceId, actionContext, options);
