...
export async function initialSyncFromPb(): Promise<void> {
  ...
  for (const id of loadedIds) {
    // Don't unload the built-in assistant plugin
    // if (id === ASSISTANT_PLUGIN_ID) continue;
  ...
  for (const instance of instances) {
    // Don't reconfigure the built-in assistant plugin
    // if (instance.id === ASSISTANT_PLUGIN_ID) continue;
    if (!enabledIds.has(instance.id)) continue;
    const record = recordById.get(instance.id);
    if (!record) continue;
    const recordConfig = record.config ?? {};
    if (configDigest(recordConfig) === configDigest(instance.config ?? {})) continue;
    try {
...
export function subscribeToPlugins(): void {
  pb.collection('plugins').subscribe<PluginRecord>(
    ...
    async (e: RealtimeEvent) => {
      ...
      if (e.action === 'create') {
        if (record.enabled) await loadPluginRecord(record);
        return;
      }

      if (e.action === 'delete') {
        if (loadedIds.has(id)) await unloadPluginById(id);
        return;
      }
