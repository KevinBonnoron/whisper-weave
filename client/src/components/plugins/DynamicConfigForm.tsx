...
function RecordField({ field, value, onChange }: RecordFieldProps) {
  ...
  const record = Object.fromEntries(
    Object.entries(raw as Record<string, unknown>).map(([k, v]) => [k, unescapeRecordValue(v)]),
  );
      ...
      <Tabs defaultValue={defaultTab}>
        ...
        {keys.map((k) => (
          <TabsContent key={k.value} value={k.value}>
            <Textarea
              placeholder={field.placeholder ?? k.label}
              value={record[k.value] ?? ''}
              onChange={(e) => onChange({ ...record, [k.value]: e.target.value })}
              rows={10}
              className="!field-sizing-fixed max-h-64 font-mono text-xs"
            />
          </TabsContent>
        ))}
      </Tabs>
